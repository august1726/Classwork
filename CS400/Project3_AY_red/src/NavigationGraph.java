...
public class NavigationGraph<NodeType,EdgeType extends Number> implements NavigationGraphADT<NodeType, EdgeType> {
    ...
    public NavigationGraph() {vertices = new Hashtable<>();}
	...
	/**
     * Insert a new undirected edge with a positive edge weight into the graph.
     * 
        ...
        for(Edge e : sourceVertex.edgesLeaving)
            if(e.target == targetVertex) {
                if(e.weight.doubleValue() == weight.doubleValue()) return false; // edge already exists
                	...
                	for (Edge e2 : targetVertex.edgesLeaving) {
                		if (e2.target.equals(sourceVertex)) {
                			e2.weight = weight;
                		}
                	}
                }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	...
        	this.dataSequence = new LinkedList<>();
        	//use a deep copy of the copyPath dataSequence, 
        	//then add the target of the new edge
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    	//First, verify that both start and end nodes are in the graph, 
        ...
        //create a new path from the starting vertex and add to queue
		Path temp = new Path(startVertex);
		queue.add(temp);
		LinkedList<Vertex> visited = new LinkedList<Vertex>(); //keeps track of visited vertices
		while (!queue.isEmpty()) {
			//remove path from queue and add its end vertex to visited
			temp = queue.poll();
			visited.add(temp.end);
			//if the path leads from the start vertex to the end, return the path
			if (temp.start.equals(startVertex) && temp.end.equals(endVertex)) {
				return temp;
			}
			for (Edge e : temp.end.edgesLeaving) {
				//if new path leads to a vertex that hasn't been visited,
				//add it to the queue
				if (!visited.contains(e.target)) {
					queue.add(new Path(temp, e));
				}
			}
		}
		//if no paths found, throw a NoSuchElementException
		throw new NoSuchElementException();
    }
	...
	public List<NodeType> getNodes() {
		LinkedList<NodeType> nodes = new LinkedList<NodeType>();
		for (Vertex v : vertices.values()) {
			nodes.add(v.data);
		}
		return nodes;
